...
Long-run research also exposes runtime quality observations. When
`long-web-research` is selected, agrun maintains `runState.researchState` as a
diagnostic snapshot:
...
- `finalAllowed` and `finalReason`: observed readiness facts for the AI and
  Inspector

Before finalization, the runtime can surface whether relevant source evidence is
thin, weak, or missing. The AI/skill owns the next move: gather more evidence,
revise the workspace, or publish honestly with limitations. The runtime no
longer overrides terminal decisions through kernel-seam hooks.
...
research observation layer: `tier`, `reason`, matched `signals`, and numeric metrics such as
...
Complex response finalization exposes workspace-readiness observations. For
research/report/long-run prompts, a workspace state that is only
`runtime-materialized` is a quality signal, not hidden runtime authorship. If the
planner tries to final/finalize before real workspace work, the runtime surfaces
a `missing_workspace_action` observation so the AI can choose a productive next
action such as `workspace_write`. Missing `final_candidate.md` is reported the
same way.
...
Long-run research reports also expose a runtime OODAE report-loop observation
before finalization. The loop is stored in `runState.researchReportLoop` and
records Observe / Orient / Decide / Act / Evaluate cycles. In the opt-in
research skill pack, a full report observation expects at least three read
sources and two relevant sources.
...
research, the runtime can surface that a topic-specific `todo_plan` is missing;
the planner/skill owns the next move. Coverage targets are then inferred from
the todo items and the topic shape. Company-like topics default toward official,
registry, and independent sources; handle/project topics default toward
official, repository, and independent sources; generic topics default toward
official and independent sources. This keeps the research loop reusable without
hardcoding a live QA topic.

When independent corroboration is missing, the report loop can expose a targeted
search suggestion for independent sources. The default observation budget is two
...
for the AI to select the same source-backed research skill, todo plan, evidence
graph, workspace-readiness observations, and limited/full report boundary.
