...
contextSnapshot.continuityResolution
```

`sessionContextView` remains as a compatibility alias for debug consumers, but `contextSnapshot` is the canonical continuity model.
