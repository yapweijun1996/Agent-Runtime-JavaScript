...
- Child returning empty `finalResponse` is a real failure mode — observed in 2026-05-26 live e2e where the child completed `status:"success"` but its planner produced no final text (Gemini grounding returned nothing). The parent surfaced "subagent completed but did not provide a summary." Follow-up needed: either force-emit a fallback finalResponse on the child, or have the parent's prompt instructions tell it how to handle empty subagent results.
