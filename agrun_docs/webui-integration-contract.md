...
- `runtime.contextSnapshot.continuityResolution`
...
- after denial, the runtime continues the action loop (planner picks an alternative), so the host should expect a new result rather than a terminal blocked state — unless the consecutive denial guard triggers, in which case the runtime force-finalizes
