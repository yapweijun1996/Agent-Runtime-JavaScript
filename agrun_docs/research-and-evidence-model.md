...
This document describes how agrun.js tracks research progress, manages evidence quality, and decides when to stop gathering information. The research model is central to the action loop's ability to answer questions that require web search and URL reading.
...
ADR-0012 is the current long-research architecture decision. This document
...
For long-run research/source-quality prompts, the runtime quality gate is
stricter than simple file presence. It blocks finalization when `evidence.json`,
`draft.md`, `critique.md`, or `final_candidate.md` is missing, and it flags
`draft_same_as_outline` / `final_candidate_same_as_outline` when those files are
...
gate. The gate observes current search/read/workspace state, evaluates evidence
readiness, records source and authority gaps, and returns structured guidance
signals to the planner/finalizer.

This gate does not own the research workflow. If the planner tries to finalize
a long-research request before creating real research tasks, the gate records a
structured signal such as `research_plan_required` and returns control to the
planner. It must not fabricate a canned `todo_plan`, fixed `web_search` query,
`read_url` target, workspace artifact, or final report.
...
Default source minimums are conservative:

- `minReadSources: 3`
- `minRelevantSources: 2` (`strong` plus `usable`/medium)
- `maxResearchLoopVetoes: 4`
- `maxIndependentSearchAttempts: 2`

If the source minimum is not met, the gate reports the missing source counts,
...
draft when evidence is exhausted. The action loop blocks repeated `web_search`
queries that already appear in the research search history, so the agent cannot
loop on the same empty template.
...
a small `runState.researchFinalEnvelope` that the AI finalizer must use when
writing the report. Runtime remains the harness gate; AI owns the report
strategy, wording, and synthesis.
...
- Full-report eligibility requires the source minimum and independent authority
  coverage. If the run has only owner-controlled/profile/repository evidence,
  or only profile/advertorial context, the runtime records limited-mode
  diagnostics internally; it must not force a user-facing
  `Research Report` title.
- The OODAE report loop keeps searching or reading while required coverage is
  missing and budget remains. Passing the numeric source minimum alone is not
  enough to finalize a full report when independent corroboration is absent.
...
## Research Continuity / Auto-Read Transition

The current continuity resolver can still automatically read URLs without asking
the planner in legacy/simple research flows. For ADR-0012 long research, that
behavior is a migration blocker, not the target architecture. Long-research
continuity should expose candidate URLs, attempted URLs, source tiers, and stop
reasons as structured state so the planner/skill decides whether to call
`read_url`.

### Selection Logic

1. Examine `aggregatedSearchResults` for unread URLs
2. Filter already-read URLs (present in `readSources`)
3. Select the most promising candidate based on ranking
4. In legacy/simple flows, create a `read_url` action decision
5. In long-research mode, expose the candidate as a gate/context signal only

### Stopping Conditions

Auto-reading stops when any of these are true:

- `autoReadAttemptCount` exceeds the configured limit
- Sufficient strong sources exist (typically 1+ `strong` tier)
- All ranked URLs have been attempted
- The `usedSummarizeLimits` flag is set (content was too large to process fully)
- `autoReadStoppedReason` is set by the continuity resolver

### Tracking

- `autoReadAttemptCount`: Incremented each time an auto-read is attempted
- `autoReadStoppedReason`: String describing why auto-reading stopped
- `usedSummarizeLimits`: Boolean flag when summarization limits were hit
...
Cycle 1: ORIENT
  ...
  → continuityResolution: null (no prior research)
  → DECIDE: planner selects web_search("agrun.js documentation")

Cycle 2: ORIENT
  → searchResults: [url1, url2, url3]
  → continuityResolution: candidate_url(url1), attempted=[], gaps=[...]
...
Cycle 3: ORIENT
  ...
  → continuityResolution: kind="finalize"
...
- Structured gate signals only; no runtime-synthesized `web_search`,
...
- 3 Mandarin prompts producing non-empty AI-authored reports with the same
  activation and gate behavior as English prompts.
