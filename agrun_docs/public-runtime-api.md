...
| `threads` | no | stable advanced | Thread routing config. `threads.intentClassifier(payload)` may return `kind: "new_task" \| "follow_up" \| "drill_down" \| "unknown"` plus existing router hints such as `targetThreadId`; `new_task` is authoritative for current-turn goal anchoring and prevents weak token overlap from inheriting the previous goal. When a classifier is configured it is also consulted to CONFIRM any structural zero-overlap divergent guess before the session is split into a new thread (AGRUN-618) — a classifier `recallIntent`/`follow_up`/`referentialIntent` verdict overrides the structural split, so recall/meta questions ("summarize this session", "what was the image I uploaded") keep or widen their context instead of landing on an empty thread; without a classifier (or on classifier failure) the structural signals remain the complete fallback. Attachment filenames are part of the routing text, so image uploads are visible to both Jaccard scoring and classifier thread summaries. |
