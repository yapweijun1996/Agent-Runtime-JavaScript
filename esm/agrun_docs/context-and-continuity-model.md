...
Layer 2: Context Snapshot (per-turn, derived)
 └ sessionMemory, inquiryContext, readAttemptSignal
...
ContextSnapshot
 ...
 │
 └ readAttemptSignal
    ├ attemptCount     — how many read_url results are already recorded
    └ threshold        — informational retry threshold shown to the AI
```
...
- Used by the planner as research context; ADR-0012 long-research action choice
  belongs to the AI/skill, not to this runtime layer
...
The runtime records search/read state and exposes a read-only `readAttemptSignal`
to the planner. The signal is advisory context only: it does not select a URL,
force a `read_url`, or force finalization. The AI/skill owns the next research
action and the decision to finalize.
...
When at least one `read_url` result exists, the next planner cycle can receive:

| Field | Meaning |
|-------|---------|
| `attemptCount` | Number of recorded read results in the current research context |
| `threshold` | Informational threshold for the AI to notice repeated reads |

The planner prompt tells the AI to choose one of the next moves itself: read a
different URL, run a fresh search, finalize with limitations, or take another
available action. The runtime does not stop the AI at the threshold; `maxSteps`
is the liveness backstop if the loop keeps retrying without progress.
