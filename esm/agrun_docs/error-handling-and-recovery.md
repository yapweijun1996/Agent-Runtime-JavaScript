...
Similarly, if the user denies `MAX_CONSECUTIVE_DENIALS` (default: 2) action approvals in a row, the runtime emits an observability-only denial streak signal. It does not force a `finalize` decision. The planner sees `deniedActions` and decides whether to pick a non-denied action, finalize honestly, or return an honest final answer explaining the limitation.
...
The guard is implemented in `approval.js` and emits an `approval-denial-streak`
step for debugging. Between denials, the planner sees `deniedActions` in its
loop state and is instructed to avoid re-selecting them.
...
- Step trace includes `approval-denial-streak` when repeated denials are observed
...
SSOT note: the `CIRCUIT_OPEN -> finalize` conversion is a current compatibility
exception in `planner.js:createCircuitOpenPlannerResult()`, not the preferred
AI-first pattern. New recovery paths should expose a structured provider
failure/observation and let the host or AI-owned planner policy choose the
terminal response; do not copy this as a general runtime-forces-finalize pattern.
