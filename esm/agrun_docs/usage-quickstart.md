...
| **Agent skill** | An instruction package (markdown SKILL.md style) the planner reads to decide what tools to call. Different from executable `skill`. Core defaults to no domain skills; opt in through `agentSkills`, `agentSkillIndexProvider`, or packages such as `@agrun/skills-research` / `@agrun/skills-coder` (see [public-runtime-api.md → Skill Loading](./public-runtime-api.md)). | `options.agentSkills` |
...
| Built-in research/coder skills are missing | Core defaults to `agentSkills: []`; no domain skill pack is loaded implicitly. | Import an opt-in pack such as `@agrun/skills-research` or `@agrun/skills-coder`, then pass its `bundledAgentSkills`. |
