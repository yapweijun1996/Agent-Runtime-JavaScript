...
- [Research Report Loop Observations](#research-report-loop-observations)
...
Planner-selectable actions ship from the runtime-owned action registry. Disable
any subset by exact action name.

| Action group | Action names | Purpose |
|--------------|--------------|---------|
| Web evidence | `web_search`, `read_url` | Search the web and fetch/extract page content through host-configured endpoints |
| Clarification | `ask_clarification` | Agent proactively asks the user a question |
| Skill catalog/tools | `list_agent_skills`, `read_agent_skill`, `use_agent_skill`, `execute_skill_tool` | Discover, read, select, and execute registered agent skill tools |
| TodoState | `todo_plan`, `todo_advance`, `todo_cancel`, `todo_run_next`, `todo_inspect` | Maintain runtime-owned task/plan state |
...
| `deepseekBrowserSkill` | DeepSeek chat completions |

```js
createRuntime({
  skills: [openaiBrowserSkill]
  // geminiBrowserSkill / deepseekBrowserSkill omitted → those providers are not available this runtime
});
```
...
A composite budget that detects common long-run stalls. Enabled by default.
Complements `maxSteps` (which is a hard ceiling) with four finer-grained caps
that emit a **convergence signal** on `runState.sessionBudget`. The signal is
surfaced to the planner via `loopState.sessionBudget`; the AI sees it and
decides how to react (switch tactics, finalize, or accept the budget). The
runtime does not force-finalize on breach — that push-mode site was removed per
ADR-0026. See [action-loop-session-loop.js:121-122](https://github.com/yapweijun1996/agrun/blob/main/0_development/src/runtime/action-loop-session-loop.js)
for the in-source tombstone.
...
2. Records the breach on `runState.sessionBudget` and surfaces the breach
   reason to the planner. The AI decides whether to switch tactics, finalize
   honestly, or continue.
...
- `maxSteps` remains the outer hard ceiling. The budget signal fires first when
  the agent is demonstrably stuck, but if the AI ignores the signal the run
  still ends through the `maxSteps` failure/continuation path rather than a
  runtime-authored final answer.
- When `budget: false`, only the legacy per-`actionName` guard is active (full back-compat path).

## Research Report Loop Observations
...
|-----------|---------|---------------|
| `researchReportLoop` | Long-form OODAE loop state, source minimum, evidence graph, and readiness signals | `runState.researchReportLoop` |
| `researchAcceptanceEvaluator` | Acceptance convergence observation showing when a clean `ready` claim conflicts with evidence gaps | `runState.researchAcceptanceEvaluator` |
...
### Activation contract — when does the observation populate?

The report-loop observation commits ONLY when at least one of these is true
(see `src/runtime/research-report-loop.js` → `shouldCommitResearchReportLoopGate`):

1. `evaluation.required === true` — this is true only when an active skill declares the relevant evidence-convergence capability, such as `requiresEvidenceConvergence`.
   - There is no legacy research-activation mode flag in the runtime request.
   - **There is NO lexical-prompt path.** Mandarin/English/any prompt text is ignored; activation is skill/capability-driven.
2. The observation was already committed on a previous step (`previous.enabled === true`).
...
For hosts that register **only host-owned data/tool skills** and never declare evidence-convergence capability, none of these should fire and `runState.researchReportLoop` stays at its idle default. `terminalRepairState.active` should remain `false`.
...
| `researchReportLoop.enabled` | `true` | `false` to short-circuit report-loop observation refresh — no evidence/source minimum observation, no acceptance signal, no `terminalRepairState` evidence/length deficits |
| `researchReportLoop.minEvidenceArtifacts` | research pack: `3`; core alone: `0` | Generic alias for the minimum evidence artifacts observed by the loop. |
| `researchReportLoop.minRelevantEvidenceArtifacts` | research pack: `2`; core alone: `0` | Generic alias for the minimum relevant evidence artifacts observed by the loop. |
| `researchReportLoop.minReadSources` | research pack: `3`; core alone: `0` | Web-research profile threshold. Legacy name for minimum evidence artifacts from successful page reads. |
| `researchReportLoop.minRelevantSources` | research pack: `2`; core alone: `0` | Web-research profile threshold. Legacy name for minimum relevant evidence artifacts. |
...
- **Hosts that ship their own evidence/quality contract** via `onBeforeFinalize` and do not need runtime source-count observations.
...
- Long-form workspace publish debug packets expose
  `candidateQualitySignal`. Node live output still uses the compatibility name
  `acceptanceGateScore` for the observed readiness score and keeps `qualityScore` as a compatibility
  alias. See [Candidate Quality Signal](./candidate-quality-signal.md).

### When to keep enabled (default)

- Hosts that expose `web_search` / `read_url` and want runtime to surface evidence-gap observations to the planner.
- Hosts that declare long-research skills and want the OODAE report-loop observation to populate `gateSignal.acceptancePacket`.
...
```js
runState.researchReportLoop // → idle defaults when the observation never commits
...
> **TL;DR.** `workspace_publish_candidate` is a publish-direct terminal — it skips the runtime finalize LLM (`usedRuntimeFinalize=false`, `tokens=0` on that step). Because skipping the finalizer is unsafe by default, the action is **gated by default**. **Three equal opt-in paths** unhide it. The gate is capability/opt-in based, not prompt-text based.
...
For hosts that bundle long-form research workflows. The gate defers automatically when the active / selected / last-read skill is `deep-research-writer` or `long-web-research`, or the active / last-read skill declares `requiresPublishReadiness`.
...
`workspace_publish_candidate` was designed for long-form publish-direct workflows where the AI drafts, finalizes, and reads-back a candidate before publishing it directly as the final answer. Because that publish path bypasses the runtime finalize LLM, it has two side effects:
...
| Option | Default | Values |
|--------|---------|--------|
| `agentSkills` | `[]` | Array of skill instruction objects |
| `agentSkillIndexProvider` | in-memory adapter for `agentSkills` | `{ listManifests, getManifest, loadSkill }` |
...
Omit `agentSkills` or pass `agentSkills: []` for no domain instructions.
...
| "Disable the research source-minimum observations for non-research hosts." | `researchReportLoop: { enabled: false }` — see [Research Report Loop Observations](#research-report-loop-observations) |
| "Allow `workspace_publish_candidate` for a host-owned publish-direct flow." | `publishCandidateGate: { enabled: false }` — see [Publish Candidate Mode Gate](#publish-candidate-mode-gate) |
...
`loop.enabled === true` themselves at every site.
