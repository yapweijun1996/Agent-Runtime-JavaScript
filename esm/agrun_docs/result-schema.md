...
| `contextSnapshot` | required | stable | Canonical structured continuity snapshot with session memory and inquiry context. Compatibility projections may still appear inside it, but they are not canonical truth. |
| `readAttemptSignal` | optional | stable | Read-only signal with `attemptCount` and informational `threshold` when `read_url` evidence exists. The AI planner owns the next research action. |
