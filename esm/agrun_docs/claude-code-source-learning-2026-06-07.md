...
> **Status note (2026-06-09):** This is a learning/planning document, not the
> current agrun runtime contract. Code sketches below must be adapted through
> ADR-0023 / ADR-0028 / ADR-0054: circuit breakers may expose observations or
> bounded protocol stops, but must not make the runtime author a final answer or
> force a research/report finalization for the AI.
...
function buildRepairActions(runState, context) {
  ...
  if (breaker.isTripped()) {
    // Circuit is open — expose a bounded stop/observation, or allow an AI-owned limited publish path.
 ...
 *         |{ type:'circuit_breaker_tripped', consecutiveFailures:number, nextAction:string }
    ...
    if (outcome.needsRepair) {
      ...
      if (!repairResult.succeeded) {
        ...
        if (circuitBreaker.isTripped()) {
          yield { type: 'circuit_breaker_tripped', consecutiveFailures: circuitBreaker.consecutiveFailures, nextAction: 'surface_observation' }
