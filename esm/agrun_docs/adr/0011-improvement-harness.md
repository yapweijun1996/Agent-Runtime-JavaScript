...
> **Status: Retired (2026-06-10).** The Phase A–D modules
> (`src/runtime/improvement.js`, `src/runtime/improvement-reflection.js`,
> their tests, and `test/fixtures/improvements/`) were removed in the
> dead-code cleanup. AGRUN-210 closed 2026-04-26; in the six weeks since,
> nothing imported the subsystem from runtime, no public API elevation
> happened, and the four reflection rules encode agrun-repo dev
> conventions (test/concerns/, withPatchedGlobals, helper symbol lists)
> that have no value inside a general host-facing runtime. The lifecycle
> it modeled now lives in task.jsonl + ADRs + the MCP KB. This document
> stays as the historical record of the design.
